...
private:
	...
	char* preSignDocument(char* docData, int docDataSize, char* docID, int64 connectionID)
	{
		...
		signer.SetNewSigLevel(PASL_BES);
		...
		if (useJSMode)
			signer.SetExternalCryptoMode(ECM_DCAUTH_JSON);
		else
		...
		{
			signer.SetWidgetAlgorithmCaption("RSA");
			signer.SetWidgetAlgorithmInfo("Strong Asymmetric Algorithm");
			signer.SetWidgetHeader("Good signature");
			signer.SetWidgetSignerCaption("Signed by: ");
			signer.SetWidgetSignerInfo("Trusted Signer");
		}
	...
	void preSigningPage(int64 connectionID)
	{
		...
		int bufSize = 0;
		char* buf = this->GetRequestBytes(connectionID, "parts[0]", &bufSize);
	...
	void completionPage(int64 connectionID)
	{
		...
		char* resp = this->GetRequestString(connectionID, "parts[0]");
	...
	void flushFile(int64 connectionID)
	{
		...
		char* docID = this->GetRequestString(connectionID, "params[0]");
...
int main(int argc, char** argv)
{
	...
	webserver.SetTLSTLSMode(SM_NO_TLS);
