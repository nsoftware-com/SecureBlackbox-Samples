...
begin
  ...
  try
    ...
    Signer.NewSignature.Level := pslBES;
...
begin
  ...
  try
    ...
    try
      FWebServer.Config('RequestFilter=parts[0]');
      Buf := FWebServer.GetRequestBytes(ConnectionID, '');
    finally
      FWebServer.Config('RequestFilter=');
    end;
...
begin
  ...
  try
    ...
    try
      FWebServer.Config('RequestFilter=params[0]');
      Resp := FWebServer.GetRequestString(ConnectionID, '');
    finally
      FWebServer.Config('RequestFilter=');
    end;
...
begin
  ...
  try
    ...
    try
      FWebServer.Config('RequestFilter=params[0]');
      ID := FWebServer.GetRequestString(ConnectionID, '');
    finally
      FWebServer.Config('RequestFilter=');
    end;
