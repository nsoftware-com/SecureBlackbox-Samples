...
void displayHelp() {
    printf(
        ...
        "              [-hashalgo hash_algorithm] [-canonmethod canon_method] [-includekey] [-detached] \n\n"
        "DESCRIPTION\n"
        ...
        "  xmlsigner -input C:\\xml\\myfile.xml -output C:\\xml\\mysignedfile.xml -cert C:\\certs\\mycert.pfx -certpass mypassword \\\n"
        "              -canonmethod 3 -hashalgo SHA1 -detached\n\n"
    );
}
...
int main(int argc, char** argv) {
    ...
    if (optext(argc, argv, "-hashalgo")) {
        signer.SetHashAlgorithm(optval(argc, argv, "-hashalgo"));
    } else {
