...
begin
  ...
  Signer.InputFile := edtInputFile.Text;
  Signer.OutputFile := edtOutputFile.Text;
  Signer.SignatureDescription := edtDescription.Text;
  Signer.SignatureURL := edtUrl.Text;
  Signer.HashAlgorithm := GetHashAlgorithm();
  if rbtIndividual.Checked then
    Signer.StatementType := acsIndividual
  else
    Signer.StatementType := acsCommercial;
  if edtSigningTime.Checked then
    Signer.ClaimedSigningTime := DateTimeToStr(edtSigningTime.DateTime)
  else
    Signer.ClaimedSigningTime := '';
  if rbtNoTimestamp.Checked then
    Signer.TimestampServer := ''
  else
  if rbtTrusted.Checked then
  begin
    Signer.TimestampType := actTrusted;
    Signer.TimestampServer := cmbTrustedServer.Text;
  end
  else
  begin
    Signer.TimestampType := actLegacy;
    Signer.TimestampServer := cmbLegacyServer.Text;
  end;
  Signer.RemoveExistingSignatures := cbxRemove.Checked;
