      ...
      MessageDlg('Please select output file', mtError, [mbOk], 0)
        ...
        MessageDlg('Your keyring does not contain private keys.' +
          ...
          MessageDlg('Your keyring does not contain public keys.' +
            ...
            'Please, select another keyring file.', mtError, [mbOk], 0)
          else if cbEncryptKeySelect.ItemIndex = -1 then MessageDlg('Please select the encryption key',
            mtError, [mbOk], 0)
            else if cbSignKeySelect.ItemIndex = -1 then MessageDlg('Please select the signing key',
              mtError, [mbOk], 0)
  else
  begin
...
begin
  ...
  begin
    ...
    cbEncryptKeySelect.Items.AddObject(Mods + GetUserFriendlyKeyName(TsbxPGPKey(pgpKeyring.PublicKeys.Item[i])), pgpKeyring.PublicKeys.Item[i]);
  end;
  ...
  begin
    ...
    cbSignKeySelect.Items.AddObject(Mods + GetUserFriendlyKeyName(TsbxPGPKey(pgpKeyring.SecretKeys.Item[i])), pgpKeyring.SecretKeys.Item[i]);
  end;
end;
