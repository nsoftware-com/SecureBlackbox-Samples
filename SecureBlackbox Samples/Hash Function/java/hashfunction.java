...
public class hashfunction extends ConsoleDemo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (optext(args, "-pass")) {
				...
				hash.setKey(cm.getKey());
			}

			if (optext(args, "-alg")) {
				hash.setAlgorithm(optval(args, "-alg"));
			} else {
				hash.setAlgorithm("SHA256");
			}

			if (optext(args, "-encoding")) {
				hash.setOutputEncoding(Integer.parseInt(optval(args, "-encoding")));
			} else {
				hash.setOutputEncoding(HashFunction.cetBase64);
			}

			// Calculate hash
			System.out.println("Calculated hash: " + new String(hash.hashFile(input), StandardCharsets.ISO_8859_1));
			System.out.println();
