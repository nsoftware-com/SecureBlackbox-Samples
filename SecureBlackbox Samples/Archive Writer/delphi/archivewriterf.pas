...
type
  ...
  private
    ...
    procedure DoRecipientFound(Sender: TObject; const PublicKeyHash: String; CertFound: Boolean);
    procedure DoDecryptionPasswordNeeded(Sender: TObject; const PasswordTarget: string; var Cancel: Boolean);
    procedure DoActionNeeded(Sender: TObject; const Description: String; var Action: Integer);
  public
    { Public declarations }
  end;
...
begin
  ...
  begin
    ...
    if FormProgress.CancelOperation then
      Application.MessageBox('Compression aborted.', 'Info', MB_OK)
    else
      Application.MessageBox('Compression finished.', 'Info', MB_OK);
    FormProgress.Hide;
...
begin
  ...
  FArchive.OnActionNeeded := DoActionNeeded;
end;
...
procedure TFormArchiveWriter.DoRecipientFound(Sender: TObject;
  const PublicKeyHash: String; CertFound: Boolean);
begin
...
procedure TFormArchiveWriter.DoActionNeeded(Sender: TObject;
  const Description: String; var Action: Integer);
begin
  if FArchive.NewArchive then
    Action := 0;
end;
