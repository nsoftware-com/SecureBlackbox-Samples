...
class signdialog extends JDialog {
	...
	private JTextField edSignatureIndex;
	...
	public static void main(String[] args) {
		try {
			signdialog dialog = new signdialog(null, 0, "", null, null);
	 ...
	 * Create the dialog.
	 */
	public signdialog(Jadesverifier _verifier, Integer sigIndex, String issuerRDN, byte[] serialNumber, byte[] subjectKeyID)
	{
		Certificatemanager manager = new Certificatemanager();

		setTitle("Signature Options");

		setBounds(100, 100, 365, 335);
		getContentPane().setLayout(new BorderLayout());
		contentPanel.setBorder(new EmptyBorder(5, 5, 5, 5));
		getContentPane().add(contentPanel, BorderLayout.CENTER);
		contentPanel.setLayout(null);

		JLabel lblSignatureName = new JLabel("Signature name:");
		lblSignatureName.setBounds(10, 23, 144, 14);
		contentPanel.add(lblSignatureName);

		edSignatureIndex = new JTextField();
		edSignatureIndex.setBounds(110, 20, 235, 22);
		edSignatureIndex.setEnabled(false);
		contentPanel.add(edSignatureIndex);

		JLabel lblIssuerRDN = new JLabel("Issuer RDN:");
		lblIssuerRDN.setBounds(10, 53, 144, 14);
		contentPanel.add(lblIssuerRDN);

		edIssuerRDN = new JTextField();
		edIssuerRDN.setBounds(110, 50, 235, 22);
		edIssuerRDN.setEnabled(false);
		contentPanel.add(edIssuerRDN);

		JLabel lblSerialNumber = new JLabel("Serial Number:");
		lblSerialNumber.setBounds(10, 83, 144, 14);
		contentPanel.add(lblSerialNumber);

		edSerialNumber = new JTextField();
		edSerialNumber.setBounds(110, 80, 235, 22);
		edSerialNumber.setEnabled(false);
		contentPanel.add(edSerialNumber);

		JLabel lblSubjectKeyID = new JLabel("Subject KeyID:");
		lblSubjectKeyID.setBounds(10, 113, 144, 14);
		contentPanel.add(lblSubjectKeyID);

		edSubjectKeyID = new JTextField();
		edSubjectKeyID.setBounds(110, 110, 235, 22);
		...
		panel_2.setBounds(5, 135, 340, 120);
		...
		btnRemove.setBounds(250, 50, 80, 25);
		...
		if (verifier != null)
		{
			edSignatureIndex.setText(sigIndex.toString());
		}
	...
	private void addCertItem(String serialNumber, String issuer) {
	...
	private void updateCertificates() {
		...
		for (int i = 0; i < verifier.getKnownCertificates().size(); i++)
		{
			...
			addCertItem(bytesToHex(verifier.getKnownCertificates().item(i).getSerialNumber()), s);
		}
	}
