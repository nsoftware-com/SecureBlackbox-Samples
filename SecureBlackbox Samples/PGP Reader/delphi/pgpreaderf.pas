...
begin
  try
    ...
    for i := 0 to Keyring.Keys.Count - 1 do
      if Keyring.Keys.Item[i].IsSecret then
        pgpReader.DecryptingKeys.Add(Keyring.Keys.Item[i]);
    pgpReader.VerifyingKeys.Clear;
    for i := 0 to Keyring.Keys.Count - 1 do
      pgpReader.VerifyingKeys.Add(Keyring.Keys.Item[i]);
...
begin
  ...
  begin
    ...
    cbVerifyKeySelect.Items.AddObject(Mods + GetUserFriendlyKeyName(TsbxPGPKey(pgpKeyring.Keys.Item[i])), pgpKeyring.Keys.Item[i]);
  end;
  ...
  begin
    ...
    begin
      ...
      cbDecryptKeySelect.Items.AddObject(Mods + GetUserFriendlyKeyName(pgpKeyring.Keys.Item[i]), pgpKeyring.Keys.Item[i]);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if cbDecryptKeySelect.ItemIndex = -1 then
