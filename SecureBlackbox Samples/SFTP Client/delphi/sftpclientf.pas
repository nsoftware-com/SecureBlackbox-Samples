...
type
  ...
  private
    ...
    function FormatRights(Entry: TsbxSFTPListEntry) : string;
  public
    { Public declarations }
  end;
...
begin
  ...
  begin
    if MessageDlg('Please confirm that you want to delete "' +
      TsbxSFTPListEntry(lvFiles.Selected.Data).Name + '"', mtConfirmation,
      [mbYes, mbNo], 0) = mrYes then
    begin
      Log('Removing item ' + TsbxSFTPListEntry(lvFiles.Selected.Data).Name);
      FName := TsbxSFTPListEntry(lvFiles.Selected.Data).Name;
      try
        if TsbxSFTPListEntry(lvFiles.Selected.Data).Directory then
        ...
        on E: Exception do
        begin
          Log('Failed to delete "' + TsbxSFTPListEntry(lvFiles.Selected.Data).Name + '", ' +
            E.Message, true);
        end;
      end;
      Refresh;
...
begin
  if FClient.Connected and Assigned(lvFiles.Selected) and Assigned(lvFiles.Selected.Data) and
    (not TsbxSFTPListEntry(lvFiles.Selected.Data).Directory) then
  begin
    SaveDialog.FileName := TsbxSFTPListEntry(lvFiles.Selected.Data).Name;
    if SaveDialog.Execute then
    begin
      Log('Downloading file ' + TsbxSFTPListEntry(lvFiles.Selected.Data).Name);
      FName := TsbxSFTPListEntry(lvFiles.Selected.Data).Name;
      Size := TsbxSFTPListEntry(lvFiles.Selected.Data).Size;
...
begin
  if Assigned(lvFiles.Selected) and Assigned(lvFiles.Selected.Data) and
    (TsbxSFTPListEntry(lvFiles.Selected.Data).Directory) then
  begin
    Log('Changing directory to ' + TsbxSFTPListEntry(lvFiles.Selected.Data).Name);
    try
      FClient.ChangeDir(TsbxSFTPListEntry(lvFiles.Selected.Data).Name);
    except
      on E : Exception do
      begin
...
var
  Info1, Info2 : TsbxSFTPListEntry;
begin
  Info1 := Item1;
  Info2 := Item2;
  if Info1.Directory = Info2.Directory then
    Result := CompareText(Info1.Name, Info2.Name)
  else
  begin
    if Info1.Directory then
      Result := -1
    else
      Result := 1;
  end;
...
begin
  ...
  try
    ...
    begin
      TsbxSFTPListEntry(FDirList.Items[I]).Name := TsbxSFTPListEntry(FDirList.Items[I]).Name;
      TsbxSFTPListEntry(FDirList.Items[I]).LongName := TsbxSFTPListEntry(FDirList.Items[I]).LongName;
    end;
    ...
    begin
      ...
      Item.Caption := TsbxSFTPListEntry(FDirList.Items[I]).Name;
      if not TsbxSFTPListEntry(FDirList.Items[I]).Directory then
      begin
        Item.SubItems.Add(IntToStr(TsbxSFTPListEntry(FDirList.Items[I]).Size));
        Item.ImageIndex := 9;
      end
      else
      begin
      ...
      Item.SubItems.Add(TsbxSFTPListEntry(FDirList.Items[I]).MTime);
      Item.SubItems.Add(TsbxSFTPListEntry(FDirList.Items[I]).Owner);
      Item.SubItems.Add(FormatRights(TsbxSFTPListEntry(FDirList.Items[I])));
...
begin
  try
    for I := 0 to lvFiles.Items.Count - 1 do
      TsbxSFTPListEntry(lvFiles.Items[I].Data).Free;
  finally
    lvFiles.Items.Clear;
  end;
end;

function TFormSftpclient.FormatRights(Entry: TsbxSFTPListEntry) : string;
begin
  Result := '';
  if Entry.Directory then
    Result := Result + 'd';
  if Entry.UserRead then
    Result := Result + 'r'
  else
    Result := Result + '-';
  if Entry.UserWrite then
    Result := Result + 'w'
  else
    Result := Result + '-';
  if Entry.UserExecute then
    Result := Result + 'x'
  else
    Result := Result + '-';
  if Entry.GroupRead then
    Result := Result + 'r'
  else
    Result := Result + '-';
  if Entry.GroupWrite then
    Result := Result + 'w'
  else
    Result := Result + '-';
  if Entry.GroupExecute then
    Result := Result + 'x'
  else
    Result := Result + '-';
  if Entry.OtherRead then
    Result := Result + 'r'
  else
    Result := Result + '-';
  if Entry.OtherWrite then
    Result := Result + 'w'
  else
    Result := Result + '-';
  if Entry.OtherExecute then
    Result := Result + 'x'
  else
    Result := Result + '-';
end;
...
var
  Item: TsbxSFTPListEntry;
begin
  Item := TsbxSFTPListEntry.Create();
  Item.Name := FClient.CurrentListEntry.Name;
  Item.LongName := FClient.CurrentListEntry.LongName;
  ...
  Item.Owner := FClient.CurrentListEntry.Owner;
  Item.CTime := FClient.CurrentListEntry.CTime;
  Item.ATime := FClient.CurrentListEntry.ATime;
  Item.MTime := FClient.CurrentListEntry.MTime;
  Item.UserRead := FClient.CurrentListEntry.UserRead;
  Item.UserWrite := FClient.CurrentListEntry.UserWrite;
  Item.UserExecute := FClient.CurrentListEntry.UserExecute;
  Item.GroupRead := FClient.CurrentListEntry.GroupRead;
  Item.GroupWrite := FClient.CurrentListEntry.GroupWrite;
  Item.GroupExecute := FClient.CurrentListEntry.GroupExecute;
  Item.OtherRead := FClient.CurrentListEntry.OtherRead;
  Item.OtherWrite := FClient.CurrentListEntry.OtherWrite;
  Item.OtherExecute := FClient.CurrentListEntry.OtherExecute;
  Item.Directory := FClient.CurrentListEntry.Directory;
