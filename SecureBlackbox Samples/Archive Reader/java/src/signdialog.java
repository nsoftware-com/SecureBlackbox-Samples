...
class signdialog extends JDialog {
	...
	public static void main(String[] args) {
		try {
			signdialog dialog = new signdialog(null, "", null);
	 ...
	 * Create the dialog.
	 */
	public signdialog(Archivereader _reader, String issuerRDN, byte[] serialNumber)
	{
		...
		panel_2.setBounds(5, 85, 340, 120);
	...
	protected void addClick()
	{
		...
		if (fileName.length() > 0)
		{
			...
			reader.getKnownCertificates().add(cert);
			updateCertificates();
		}
	}
	...
	private void updateCertificates() {
		...
		for (int i = 0; i < reader.getKnownCertificates().size(); i++)
		{
			String s = reader.getKnownCertificates().item(i).getIssuer();
			if (s == "")
				s = "<unknown>";

			addSignCertItem(bytesToHex(reader.getKnownCertificates().item(i).getSerialNumber()), s);
		}
	}

	protected void removeClick()
	{
		reader.getKnownCertificates().remove(table.getSelectedRow());
