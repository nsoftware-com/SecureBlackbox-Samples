...
type
  ...
  private
    ...
    procedure DoSignatureFound(Sender: TObject; const IssuerRDN: String;
      const SerialNumber: TBytes; const SubjectKeyID: TBytes; CertFound: Boolean;
...
begin
  ...
  if FormProgress.CancelOperation then
    Application.MessageBox('Extraction aborted.', 'Info', MB_OK)
  else
    Application.MessageBox('Extraction finished.', 'Info', MB_OK);
  FormProgress.Hide;
end;
...
begin
  ...
  try
    FArchive.Overwrite := true;
    FArchive.ExtractAll(Destination);
  except
    on E : Exception do
      FormProgress.AddToLog('Error : ' + E.Message);
  end;
